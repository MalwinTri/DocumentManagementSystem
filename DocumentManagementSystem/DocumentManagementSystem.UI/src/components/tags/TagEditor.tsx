...
import { Tag as TagIcon, Building2, Calendar, Hash } from "lucide-react";
...
export function TagEditor({
    ...
    return (
        <div className="space-y-2" ref={boxRef}>
            {/* Quick prefixes (neues Design) */}
            <div className="flex flex-wrap items-center gap-2">
                <span className="text-xs text-muted-foreground mr-1">Quick:</span>

                {[
                    { k: "type:", label: "Type", Icon: TagIcon },
                    { k: "org:", label: "Org", Icon: Building2 },
                    { k: "year:", label: "Year", Icon: Calendar },
                    { k: "kw:", label: "Keyword", Icon: Hash },
                ].map(({ k, label, Icon }) => (
                    <button
                        key={k}
                        type="button"
                        onClick={() => quickPrefix(k)}
                        className="inline-flex items-center gap-1.5 rounded-full bg-muted px-2.5 py-1 text-xs text-muted-foreground hover:bg-muted/70 hover:text-foreground transition"
                        title={k}
                    >
            ...
            <div className="text-xs text-muted-foreground">
                Beispiele: <span className="font-medium">org:ÖBB</span>,{" "}
